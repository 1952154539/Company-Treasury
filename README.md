...
甲方是一家管理数亿美元资产的 Web3 投资机构（DAO），日常运营涉及多部门资金调配、合作方付款、员工薪资发放、闲置资金理财等场景。原有流程存在以下问题：

| 痛点 | 具体表现 |
|------|---------|
| **审批不透明** | 大额转账依赖 CEO/CFO 离线签名（微信/Telegram 确认），无链上留痕，出事后无法追溯"谁在什么时间批准了哪笔交易" |
| **资金效率低** | 数千万 USDC 长期闲置在冷钱包中，无利息收入；如需理财，需手动将资金转入 CEX/DeFi 协议，操作繁琐且脱离多签管控 |
| **预算失控** | 部门预算审批通过后，资金一次性打入部门钱包，实际使用情况无法追踪，存在"审批时说 A、实际花在 B"的挪用风险 |
| **支付管理混乱** | 员工工资、供应商分期款、Grant 拨款等周期性支付依赖人工记录和手动打款，经常遗漏或延迟，且无链上支付凭证 |
| **应急响应慢** | 遇到安全事件（如私钥泄露、协议漏洞），缺乏分级的应急机制，要么反应过度（全部冻结影响正常业务），要么反应不足（损失扩大） |

### 项目目标

为甲方构建一套**链上企业金库系统**，实现以下目标：

1. **资金安全第一**：所有资产由智能合约托管，任何资金流出必须通过 N/M 多签 + 时间锁双重保障，私钥泄露也无法单方面转走资产
2. **审批全链路可追溯**：每笔交易的提案、审批、执行均在链上记录，支持 EIP-712 链下签名（gasless），审批人无需支付 gas
3. **闲置资金自动理财**：金库资金可存入 ERC-4626 收益策略（Aave / Lido / Compound），收益自动归集，支持一键调仓和紧急撤出
4. **预算冻结-结算机制**：部门预算审批通过后资金仍在金库中，实际支出时通过多签提案冻结 → 执行 → 结算，防止挪用和双花
5. **流支付替代周期性打款**：采用 Sablier 式线性释放，工资/拨款随时间自动解锁，收款方主动提现，取消后剩余资金自动退回
6. **三级应急体系**：Pause（暂停提案）→ Shutdown（冻结全部转账）→ Recovery（48h 延迟提取），分级响应不同级别安全事件，既不过度也不不足

### 方案选型

甲方最初考虑直接使用 Gnosis Safe 多签 + 人工管理，但经评估后发现：

- Gnosis Safe 无法实现部门预算的冻结-结算模型
- 无法在 Safe 内部直接对接 DeFi 收益协议并保持多签管控
- 缺乏流支付和周期性付款的自动化能力
- 应急响应粒度不够（Safe 只能禁用某个 owner，无法分级控制）

最终决定基于 Solidity 自研金库系统，以 UUPS 可升级代理 + Hub-and-Spoke 模块化架构实现上述全部需求。该系统上线后管理资产规模约 **$50M+**，日均处理 20+ 笔多签交易。
