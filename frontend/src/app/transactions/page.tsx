...
import { useTransaction, useTransactionApprovals, useSignerList, useIsPaused } from "@/hooks/useTreasury";
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">多签交易</h1>
          <p className="text-sm text-gray-500 mt-1">最近交易</p>
        </div>
        <ProposeForm />
      </div>
      ...
      {!isConnected ? (
        <div className="text-center py-10 text-gray-500">请连接钱包查看交易</div>
      ) : count === 0 ? (
        <div className="text-center py-10 text-gray-500">暂无交易</div>
      ) : (
  ...
  return (
    <div className="border-t border-gray-100 p-4 space-y-4">
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div><span className="text-gray-500">目标地址:</span> <span className="font-mono text-gray-700">{(t.target as string)?.slice(0, 10)}...</span></div>
        ...
        <div><span className="text-gray-500">关联预算:</span> <span className="font-mono text-gray-700">{(t.budgetId as string)?.slice(0, 10)}...</span></div>
      </div>
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 w-full max-w-md">
      ...
      <div className="space-y-2">
        ...
        <input type="text" placeholder="预算ID (可选)" value={budgetId}
          ...
          className="w-full px-3 py-1.5 border border-gray-300 rounded text-sm font-mono" />
        <button onClick={handlePropose} disabled={!target}
